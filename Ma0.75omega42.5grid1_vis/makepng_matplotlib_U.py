#!/usr/bin/env python3
"""
使用 matplotlib 和 fluidfoam 库可视化 OpenFOAM 速度场数据
"""
import os
import numpy as np
...
if __name__ == '__main__':
    ...
    # 创建输出目录
    output_dir = 'matplotlib_output_U'
    ...
    # 设置速度范围（速度大小）
    U_min = 0.0
    U_max = 350.0  # 根据马赫数0.75，声速约340m/s，最大速度约255m/s，留一些余量
    ...
    for idx, ts in enumerate(timeStepList):
        ...
        # 读取速度场数据
        try:
            Ux, Uy, Uz = readvector('.', str(ts), 'U')
            # 计算速度大小
            U_mag = np.sqrt(Ux**2 + Uy**2 + Uz**2)
            print(f"  ✓ 读取速度数据: {len(U_mag)} 个单元")
            print(f"  速度大小范围: {U_mag.min():.2f} 到 {U_mag.max():.2f} m/s")
        except Exception as e:
            print(f"  ✗ 读取速度数据失败: {e}")
            continue
        ...
        try:
            ...
            scatter = ax.scatter(plot_x, plot_y, c=U_mag, cmap='jet', s=5,
                               vmin=U_min, vmax=U_max, edgecolors='none')
            ax.set_xlabel(xlabel, fontsize=12)
            ax.set_ylabel(ylabel, fontsize=12)
            ax.set_title(f'Velocity Magnitude (t = {ts:.6f} s)', fontsize=14, fontweight='bold')
            ...
            filename = os.path.join(output_dir, f'U_deg{str(ts).replace(".", "")}.png')
            plt.tight_layout()
        ...
        contourf = ax.tricontourf(triang, U_mag, levels=levels, cmap='jet',
                                  vmin=U_min, vmax=U_max, extend='both')

        # 可选：添加等值线
        contour = ax.tricontour(triang, U_mag, levels=10, colors='black',
